...
struct CheckoutView: View {
    ...
    private func createOrders() {
        ...
        do {
            ...
            // 上傳訂單到 Firebase（非阻塞）
            Task {
                await FirebaseManager.shared.uploadOrders(newOrders)
                print("🔥 已成功上傳 \(newOrders.count) 筆訂單到 Firebase")
            }
            
            // 發送本地推播通知
            if newOrders.count == 1 {
                notificationManager.sendOrderConfirmationNotification(
                    orderNumber: newOrders.first!.orderNumber,
                    customerName: customerName
                )
            } else {
                // 多筆訂單時發送批量通知給客戶
                let content = UNMutableNotificationContent()
                content.title = "🌸 批量訂單確認"
                content.body = "親愛的 \(customerName)，您的 \(newOrders.count) 筆訂單已成功建立！訂單編號：\(orderNumbers.joined(separator: ", "))"
                content.sound = .default
                content.badge = NSNumber(value: newOrders.count)
                content.categoryIdentifier = "BATCH_ORDER_CONFIRMATION"
                
                let trigger = UNTimeIntervalNotificationTrigger(timeInterval: 1, repeats: false)
                let request = UNNotificationRequest(
                    identifier: "customer_batch_orders_\(Date().timeIntervalSince1970)",
                    content: content,
                    trigger: trigger
                )
                ...
                UNUserNotificationCenter.current().add(request) { error in
                    if let error = error {
                        print("❌ 客戶批量訂單通知發送失敗: \(error.localizedDescription)")
                    } else {
                        print("✅ 客戶批量訂單通知已發送")
                    }
                }
                ...
                // 發送業主批量訂單通知
                notificationManager.sendMerchantBatchOrderNotification(
            ...
            print("✅ 已成功建立 \(newOrders.count) 筆訂單")
        } catch {
